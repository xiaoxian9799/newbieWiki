...
- The `subprocess.Popen()` Function - (Process)
    - The `poll()` Method - returns `None` if the process is stil running at the time `poll()` is called, else returns the process's integer `exit code` if the program has terminated
    - The `wait()` Method - block until the launced process has terminated

An exit code is used to indicate whether the process terminated without errors (an exit code of 0) or whether an error caused the process to terminate (a nonzero exit code).
